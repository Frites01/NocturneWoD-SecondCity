	...
	if(!real_bans_only && !C && CONFIG_GET(flag/usewhitelist))
		if(!check_whitelist(ckey))
			if (admin)
				...
				return list("reason"="whitelist", "desc" = CONFIG_GET(string/missing_whitelist_message)) // NOCTURNE EDIT - SQL-based whitelist. ORIGINAL: return list("reason"="whitelist", "desc" = "\nReason: You are not on the white list for this server")
